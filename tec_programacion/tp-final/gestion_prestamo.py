...
def validar_ISBN():
    ...
    while valido:
        try:
            identificador = input(f"Ingrese el ISBN(sin puntos): ")
            identificador_int = int(identificador)
            if len(identificador) == long_1:
                valido = False
            else:
                print("El numero ingresado debe contener 13 numeros")
        except:
...
def validar_dni():
    ...
    while valido:
        try:
            dni = input(f"Ingrese dni de cliente(sin puntos): ")
            identificador_int = int(dni)
            if len(dni) == 7 or len(dni) == 8:
                valido = False
            elif len(dni) < 7:
                print("El numero ingresado es muy corto")
            else:
                print("El numero ingresado en muy largo")
        except:
            print("Ingrese un valor numerico valido")
    return dni
